...
if prompt_yes_no "Push image to registry" n; then
	SHOULD_PUSH=1
else
	SHOULD_PUSH=0
fi
...
# If the build produced/preserved a manifest list locally, ask whether to push it
if [ "$SHOULD_PUSH" -eq 1 ]; then
	echo "Pushing $TAG to registry..."
	docker push "$TAG"
else
	echo "Skipping push (you can push manually with: docker push $TAG)"
fi
